...
import occ.cs272.ic10.LinkedList_2.SLLIterator;

// Some generic methods and iterators
public class IteratorDemo
{
    // TODO: write the method addBack(Sequence<T> list, T ... args)
    public static <T> void addBack(Sequence<T> list, T ... args) {
        Iterator<T> iter = list.iterator();
        for (T e : args) list.add(iter, e);
    }
    
    // TODO: write the method addFront(Sequence<T> list, T ... args)
    public static <T> void addFront(Sequence<T> list, T ... args) {
        for (T e : args) list.add(list.iterator(), e);
    }

    // TODO: write the method insertBefore(Sequence<T> list, T target, T element)
    public static <T> void insertBefore(Sequence<T> list, T target, T element) {
        Iterator<T> iter;
        for (iter = list.iterator(); iter.hasNext(); iter.next()) {
            if (target.equals(iter.get())) {
    ...
    public static <T> T remove(Sequence<T> list, T target) {
        Iterator<T> iter;
        for (iter = list.iterator(); iter.hasNext(); iter.next()) {
            if (target.equals(iter.get())) {
                T result = iter.get();
